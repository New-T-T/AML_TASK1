...
def select_features(df_original: pd.DataFrame, target_original: pd.DataFrame, alpha: float, verbose: bool = True,
                    timing: bool = True, seed: int = 40) -> pd.DataFrame:
    X_train, X_test, y_train, y_test = preprocess(df_original=df_original, target_original=target_original, # TODO: remove train-test
    ...
    print(f"{'':<1} Lasso test r2 score: {colorama.Fore.GREEN}{r2_score(y_test, y_pred)}{colorama.Style.RESET_ALL}")
...
def dummy_rfe_regression(X_train: pd.DataFrame, y_train: pd.DataFrame, X_test: pd.DataFrame, y_test: pd.DataFrame, verbose: bool = True, timing: bool = True) -> pd.DataFrame:
    start_time = time.process_time()
    ...
    if verbose:
        ...
        print(f"{'':<1} RandomForestRegressor r2 score: {colorama.Fore.GREEN}{r2_score(y_test, y_pred)}{colorama.Style.RESET_ALL}")
        print(f"{'':<1} RandomForestRegressor mse: {colorama.Fore.GREEN}{mean_squared_error(y_test, y_pred)}{colorama.Style.RESET_ALL}")
